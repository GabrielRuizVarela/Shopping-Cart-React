...
class GLTFMaterialsEmissiveStrengthExtension {
  ...
  extendMaterialParams(materialIndex, materialParams) {
    ...
    const { emissiveStrength } = materialDef.extensions[this.name];
    ...
    if (bufferView.extensions && bufferView.extensions[this.name]) {
      ...
      return buffer.then((res) => {
        ...
        if (decoder.decodeGltfBufferAsync) {
          return decoder.decodeGltfBufferAsync(count, stride, source, extensionDef.mode, extensionDef.filter).then((res) => res.buffer);
        }
...
class GLTFParser {
  ...
  getDependency(type, index) {
    ...
    if (!dependency) {
      switch (type) {
        ...
        case 'animation':
          dependency = this._invokeOne((ext) => ext.loadAnimation && ext.loadAnimation(index));
          break;
  ...
  loadBuffer(bufferIndex) {
    ...
    return new Promise((resolve, reject) => {
      loader.load(LoaderUtils.resolveURL(bufferDef.uri, options.path), resolve, undefined, () => {
  ...
  loadImageSource(sourceIndex, loader) {
    ...
    if (sourceDef.bufferView !== undefined) {
      ...
      sourceURI = parser.getDependency('bufferView', sourceDef.bufferView).then((bufferView) => {
        isObjectURL = true;
    ...
    const promise = Promise.resolve(sourceURI).then((sourceURI) => new Promise((resolve, reject) => {
      let onLoad = resolve;
  ...
  loadNode(nodeIndex) {
    ...
    return (function () {
      ...
      const meshPromise = parser._invokeOne((ext) => ext.createNodeMesh && ext.createNodeMesh(nodeIndex));
