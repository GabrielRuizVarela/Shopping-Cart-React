...
class FBXTreeParser {
  ...
  loadTexture(textureNode, images) {
    ...
    const { children } = connections.get(textureNode.id);
  ...
  parseMorphTargets(relationships, deformerNodes) {
    ...
    for (let i = 0; i < relationships.children.length; i++) {
      ...
      rawMorphTarget.geoID = connections.get(parseInt(child.ID)).children.filter((child) => child.relationship === undefined)[0].ID;
  ...
  setLookAtProperties(model, modelNode) {
    if ('LookAtProperty' in modelNode) {
      const { children } = connections.get(model.ID);
  ...
  bindSkeleton(skeletons, geometryMap, modelMap) {
    ...
    for (const ID in skeletons) {
      ...
      const { parents } = connections.get(parseInt(skeleton.ID));
...
class AnimationParser {
  ...
  parseAnimStacks(layersMap) {
    ...
    for (const nodeID in rawStacks) {
      const { children } = connections.get(parseInt(nodeID));
...
function generateTransform(transformData) {
  ...
  let lTransform = lTranslationM.clone().multiply(lRotationOffsetM).multiply(lRotationPivotM).multiply(lPreRotationM)
    .multiply(lRotationM)
